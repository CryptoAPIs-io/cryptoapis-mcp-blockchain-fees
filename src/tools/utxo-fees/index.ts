...
• estimate-transaction-smart-fee: Estimate the fee rate needed for confirmation within a target number of blocks`,
    ...
    handler: (client: CryptoApisHttpClient, logger: McpLogger) => async (input: UtxoFeesToolInput) => {
        ...
        logger.logInfo({ tool: "blockchain_fees_utxo", action: input.action, blockchain: input.blockchain, network: input.network, creditsConsumed: result.creditsConsumed, creditsAvailable: result.creditsAvailable, responseTime: result.responseTime, throughputUsage: result.throughputUsage });
