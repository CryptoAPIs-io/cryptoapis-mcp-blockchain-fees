...
const CRYPTOAPIS_SERVER_INFO = {
    ...
    version: "0.1.0",
    title: "CryptoAPIs Blockchain Fees",
...
export async function startBlockchainFeesServer(opts: StartOptions) {
    ...
    if (opts.transport === "stdio") {
        ...
        logger.logInfo("cryptoapis-blockchain-fees MCP running (stdio)");
        return;
    }
    ...
    app.listen(port, host, () => {
        logger.logInfo(`cryptoapis-blockchain-fees MCP running (http) at http://${host}:${port}${path}`);
        if (cfg.apiKey) {
